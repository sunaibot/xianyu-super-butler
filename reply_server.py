from fastapi import FastAPI, HTTPException, Depends, status, UploadFile, File, Form, Body, Query, WebSocket, WebSocketDisconnect, Request
...
webhook_test_rate_limit = make_rate_limiter(max_requests=5, window_seconds=60, key_prefix='webhook_test')
...
async def change_user_password(request: ChangePasswordRequest, current_user: Dict[str, Any] = Depends(get_current_user), _: bool = Depends(change_password_rate_limit)):
    from db_manager import db_manager
...
def update_system_setting(key: str, setting_data: SystemSettingIn, _: None = Depends(require_auth)):
    ...
    try:
        ...
        if success:
            webhook_keys = {'webhook_enabled', 'webhook_url', 'webhook_secret', 'webhook_events'}
...
def _validate_webhook_url(url: str) -> None:
    ...
    # 禁止常见内网域名后缀
    blocked_suffixes = ('.local', '.internal', '.localhost', '.cluster.local', '.svc')
...
async def test_webhook(test_data: dict, _: None = Depends(require_auth), __: bool = Depends(webhook_test_rate_limit)):
    """测试 Webhook 连接"""
