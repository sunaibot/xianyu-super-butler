...
import {
  Plus, Power, Edit2, Trash2, QrCode, Check, Loader2,
  MessageSquare, RefreshCw, Save, User, Clock, MessageCircle,
  Upload, Key, Eye, EyeOff, Bot, Settings, Box, ShoppingBag
} from 'lucide-react';
...
const AccountList: React.FC = () => {
  ...
  // QR 登录轮询定时器引用，用于在卸载/关闭弹窗时清理，避免内存泄漏与无效请求
  ...
  const handleDelete = async (id: string) => {
    if (await confirm({ title: '确认删除账号', content: '确认删除该账号吗？此操作不可恢复。', variant: 'danger' })) {
  ...
  const openEditModal = async (account: AccountDetail) => {
    ...
    try {
      ...
      setEditForm({
        remark: data.remark || account.remark || account.note || '',
        cookie: data.cookie || data.value || account.cookie || account.value || '',
        auto_confirm: data.auto_confirm ?? account.auto_confirm ?? false,
        pause_duration: data.pause_duration ?? account.pause_duration ?? 0,
        username: data.username || account.username || '',
        login_password: account.login_password || '',
        show_browser: data.show_browser ?? account.show_browser ?? false,
        showLoginPassword: false,
      });
    } catch (e) {
      console.error('Failed to load account details for edit:', e);
      setEditForm({
  ...
  const startQRLogin = async () => {
    ...
    try {
      ...
      if (res.success && res.qr_code_url && res.session_id) {
        ...
        qrPollingRef.current = setInterval(async () => {
          try {
            ...
            // 单次状态查询失败不终止轮询，仅在控制台记录
            console.error('QR 状态查询失败:', err);
          }
        }, 2000);
      } else {
        setQrStatus('error');
      ...
      setQrStatus('error');
      showToast('error', '扫码登录启动失败');
    }
  };

  // 关闭二维码弹窗时停止轮询，避免后台无效请求
  const closeQRModal = () => {
    stopQRPolling();
    setShowQRModal(false);
  };
        ...
        {accounts.map((account) => (
          <div key={account.id} className="ios-card p-6 rounded-[2rem] flex items-center justify-between group hover:border-[#FFE815] transition-all duration-300">
            ...
            <div className="flex items-center gap-3">
                <button
                    onClick={() => navigate('items', { filter: { cookie_id: account.id } })}
                    className="p-3 rounded-xl hover:bg-yellow-50 transition-colors text-yellow-600 min-h-[44px] flex items-center justify-center"
                    title="查看该账号的商品"
                >
                    <Box className="w-5 h-5" />
                </button>
                <button
                    onClick={() => navigate('orders', { filter: { cookie_id: account.id } })}
                    className="p-3 rounded-xl hover:bg-orange-50 transition-colors text-orange-600 min-h-[44px] flex items-center justify-center"
                    title="查看该账号的订单"
                >
            ...
            <div>
              ...
              {editLoading ? (
                <div className="w-full h-32 bg-gray-50 rounded-xl flex items-center justify-center">
                  ...
                  <span className="ml-2 text-sm text-gray-500">加载Cookie中...</span>
                </div>
              ) : (
                <textarea
