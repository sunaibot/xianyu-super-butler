...
def _notify_event(event_type: str, data: dict = None):
    ...
    try:
        ...
        if loop.is_running():
            loop.create_task(event_bus.broadcast(event_type, data))
        else:
            asyncio.run(event_bus.broadcast(event_type, data))
    except Exception:
        pass
...
class XianyuLive:
            ...
            async with order_lock:
                ...
                try:
                    ...
                    if delivery_contents:
                        ...
                        # 推送发货完成事件
                        try:
                            _notify_event('delivery_completed', {
                                'order_id': order_id,
                                'item_id': item_id,
                                'cookie_id': self.cookie_id,
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            # 推送消息接收事件
            try:
                _notify_event('message_received', {
                    'cookie_id': self.cookie_id,
                    'timestamp': int(time.time() * 1000)
                })
            except Exception:
